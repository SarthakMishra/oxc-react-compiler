...
#### Stage 4e: UPSTREAM ERROR Fixture Handling (was 29 "Found 1 error", now 15 remaining after Stage 4f Groups A+C)
...
**Latest gains (2026-04-03, +14 from Stage 4f Groups A+C, 526->540):** See Stage 4f below for details.

**Previous gains (2026-03-26, +4 from 495->499):**
...
**Key correction (2026-04-03):** After Stage 4f Groups A+C (+14, try-catch value block bail + computed key bail), the WE-COMPILE-THEY-DON'T pool dropped from 108 to ~94. Of these, **15 "Found 1 error" fixtures remain** (was 29, -10 try-catch, -4 computed key). The 15 remaining are scattered across 7+ categories (mutation, PruneHoistedContexts, optional chains, context variables, post-render mutation, preserve-memo, hoisting). Error.* fixtures in known-failures: ~12 remaining (unchanged by this session's work, which targeted non-error.* fixtures).
...
**Revised path to 600 (updated 2026-04-03, 540 base):** Now only need +60 to reach 600. Most actionable non-scope-inference work: Stage 4f remaining (15 "Found 1 error" bails, +10-15, zero regression risk). Scope inference fixes (Stage 3, +50-100) remain the only path for large gains. Stage 2i preserve-memo bails DEFINITIVELY BLOCKED (4 approaches, worst -56). Stage 4b validateInferredDep remaining 29 BLOCKED by scope dep resolution. B2 scope-inference dependent. 0-slot codegen REJECTED (-52). **Session gains: +44 total (preserve-memo default +30, try-catch bail +10, computed key bail +4). Session attempt: temp-name-skipping -56 reverted.**
...
**Key learning from 2026-04-03 session (496->540, +44):**
- **Enabling preserve-memo validation by default was a massive free win (+30).** The test harness was not passing `validatePreserveExistingMemoizationGuarantees: true` to the compiler by default. Adding it to the Environment config immediately gained 30 fixtures because many error.* fixtures that should bail are now correctly caught by the preserve-memo validation.
- **AST-level bail-outs are the safest conformance gains.** Try-catch value block detection (+10) and object expression computed key detection (+4) are both pre-lowering AST walks in `program.rs` that bail before any HIR construction. Zero regression risk because they only add bail paths, never change compilation output.
- **Temp-name-skipping is DEFINITIVELY not viable (-56).** This was the 4th attempt at reducing preserve-memo false-positive bails via filtering. The -56 regression (worst of all 4 attempts) confirms that ALL skip/filter approaches are fundamentally flawed because error.* fixtures depend on the validation firing on temp-named deps. The only viable path is fixing scope dep resolution (SSA temp -> named variable mapping).
- **"Found 1 error" fixtures are 19 categories deep.** The remaining 15 fixtures are scattered across mutation, PruneHoistedContexts, optional chains, context variables, etc. Each requires a different fix. No single implementation gains more than 3-4.
- **WE-COMPILE-THEY-DON'T pool shrunk from 141 to 94.** After this session's 14 fixtures (10 try-catch + 4 computed key) moved from "we compile" to "we bail = pass."
...
- **Nearly ALL remaining conformance gains require scope inference.** After exhaustive analysis of all ~1177 diverged fixtures: slots-DIFFER (~579) = scope inference, slots-MATCH (~240) = scope inference + naming, both-no-memo (~90) = 0-slot codegen (scope inference), we-compile-they-don't (~94) = 69 scope surplus + 15 "Found 1 error" + 10 flow parse, we-bail-they-compile (~174) = 83 preserve-memo BLOCKED + rest various. The non-scope-inference work remaining: Stage 4f Groups B/D/E/F/H (15 "Found 1 error" bails, +10-15) and 3 remaining Infer mode fixtures (directive support). Stage 2i preserve-memo DEFINITIVELY BLOCKED (4 attempts, all net-negative).
