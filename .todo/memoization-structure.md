...
- **Mutation aliasing ranges rewrite (2026-03-14)**: `infer_mutation_aliasing_ranges.rs` has been rewritten to use graph-based BFS mutation propagation (see [over-memoization-bailout.md](over-memoization-bailout.md)#gap-5-mutation-aliasing-bail-out). This is a structural prerequisite for fixing scope over-splitting -- the previous flat one-level alias traversal could not propagate mutations transitively through multi-hop alias chains, potentially causing scopes to split where they should merge. The BFS algorithm matches upstream's temporal-index-guarded traversal.
