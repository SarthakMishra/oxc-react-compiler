use crate::hir::types::{
    DeclarationId, HIR, IdentifierId, InstructionValue, ReactiveScope, ReactiveScopeDeclaration,
...
///
/// After `infer_reactive_scope_variables` assigns scope IDs, some instructions
/// that produce values exclusively consumed within one scope are NOT members of
/// that scope (their mutable range is trivial). This pass identifies those
/// orphans and pulls them into the consuming scope.
///
/// Algorithm:
/// 1. Build a map from each instruction's lvalue ID to its scope (if any)
/// 2. For each unscoped instruction, check if ALL consumers are in the same scope
/// 3. If yes, assign the producing instruction to that scope
/// 4. Propagate transitively with a fixpoint loop
///
/// This must run after Pass 33 (infer_reactive_scope_variables) and before
/// Pass 34 (memoize_fbt_and_macro_operands_in_same_scope).
pub fn propagate_scope_dependencies_hir_prepass(hir: &mut HIR) {
    use crate::reactive_scopes::infer_reactive_scope_variables::collect_operand_ids;

    // Phase 1: Build ID → scope mapping from current HIR state
    let mut id_to_scope: FxHashMap<IdentifierId, Box<ReactiveScope>> = FxHashMap::default();
    for (_, block) in &hir.blocks {
        for instr in &block.instructions {
            if let Some(ref scope) = instr.lvalue.identifier.scope {
                id_to_scope.insert(instr.lvalue.identifier.id, scope.clone());
            }
        }
    }

    // Phase 2: Build producer → consumers map.
    // For each instruction, record which IDs consume (use) the lvalue ID.
    ...
    for (_, block) in &hir.blocks {
        for instr in &block.instructions {
            ...
            for op_id in operand_ids {
                consumers.entry(op_id).or_default().push(lvalue_id);
            }
        }
    }

    // Phase 3: Fixpoint — pull unscoped instructions into consuming scope
    ...
    while changed {
        ...
        for (_, block) in &hir.blocks {
            for instr in &block.instructions {
                ...
                if id_to_scope.contains_key(&lvalue_id) {
                    continue;
                }

                // Find all consumers of this instruction's lvalue
                let Some(consumer_ids) = consumers.get(&lvalue_id) else {
                    continue;
                };

                // Check if ALL consumers are in the SAME scope
                let mut target_scope: Option<ScopeId> = None;
                let mut all_same = true;
                for consumer_id in consumer_ids {
                    if let Some(scope) = id_to_scope.get(consumer_id) {
                        match target_scope {
                            None => target_scope = Some(scope.id),
                            Some(existing) if existing == scope.id => {}
                            _ => {
                                all_same = false;
                                break;
                            }
                        ...
                        // Consumer is also unscoped — can't determine target yet,
                        // but don't reject. We'll catch it in a later iteration.
                        // For now, skip this producer.
                        all_same = false;
                        break;
                    }
                }

                if all_same && target_scope.is_some() {
                    // Find the scope object from any consumer
                    if let Some(scope) = consumer_ids.iter().find_map(|cid| id_to_scope.get(cid)) {
    ...
    // Phase 4: Apply scope assignments back to the HIR
    for (_, block) in &mut hir.blocks {
        for instr in &mut block.instructions {
            let lvalue_id = instr.lvalue.identifier.id;
            if instr.lvalue.identifier.scope.is_none()
                && let Some(scope) = id_to_scope.get(&lvalue_id)
            {
                instr.lvalue.identifier.scope = Some(scope.clone());
            }
        }
    }
}
